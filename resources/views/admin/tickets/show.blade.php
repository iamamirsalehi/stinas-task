...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg p-8 mb-6">
        <div class="flex justify-between items-start mb-6">
            <div>
                <h1 class="text-2xl font-semibold mb-2">{{ $ticket->title ?? 'Sample Ticket Title' }}</h1>
                <div class="flex items-center gap-4 text-sm text-[#706f6c] dark:text-[#A1A09A]">
                    <span>Created by: <strong>{{ $ticket->user->name ?? 'John Doe' }}</strong></span>
                    <span>•</span>
                    <span>{{ $ticket->created_at ?? now()->format('M d, Y H:i') }}</span>
                </div>
            </div>
            <div>
                @php
        ...
        <div class="mb-6">
            ...
            <div class="text-sm text-[#706f6c] dark:text-[#A1A09A] whitespace-pre-wrap bg-[#FDFDFC] dark:bg-[#0a0a0a] p-4 rounded-sm border border-[#e3e3e0] dark:border-[#3E3E3A]">
                {{ $ticket->description ?? 'This is a sample ticket description. It contains details about the issue or request.' }}
            </div>
        </div>

        @if($ticket->file ?? false)
            <div class="mb-6">
                <h2 class="text-lg font-medium mb-2">Attachment</h2>
                <a 
                    href="{{ asset('storage/' . ($ticket->file ?? 'files/sample.pdf')) }}" 
                    target="_blank"
    ...
    @if(($ticket->status ?? 'pending') === 'pending')
        <div class="bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg p-8">
            ...
            <form method="POST" action="{{ route('admin.tickets.approve', $ticket->id ?? 1) }}" class="mb-6">
                @csrf
                <div class="mb-4">
            ...
            <form method="POST" action="{{ route('admin.tickets.reject', $ticket->id ?? 1) }}">
                @csrf
                <div class="mb-4">
