...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white dark:bg-[#161615] shadow-[inset_0px_0px_0px_1px_rgba(26,26,0,0.16)] dark:shadow-[inset_0px_0px_0px_1px_#fffaed2d] rounded-lg p-8">
        ...
        <form method="POST" action="{{ route('dashboard.tickets.create') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-6">
                <label for="file" class="block text-sm font-medium mb-2">Attachment <span class="text-[#f53003] dark:text-[#FF4433]">*</span></label>
                <input 
                    type="file" 
                    id="file" 
                    name="attachment" 
                ...
                >
                <p class="mt-1 text-xs text-[#706f6c] dark:text-[#A1A09A]">Accepted formats: PDF, JPG, JPEG, PNG</p>
                @error('file')
                    <p class="mt-1 text-sm text-[#f53003] dark:text-[#FF4433]">{{ $message }}</p>
                @enderror
            </div>
